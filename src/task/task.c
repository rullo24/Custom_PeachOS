...
struct task *task_head = 0; // task linked list head
...
int copy_string_from_task(struct task *task, void *virtual, void *phys, int max) {
    ...
    int res = 0;
    char *tmp = kzalloc(max); // memory shared by task and kernel page tables
    if (!tmp) {
        res = -ENOMEM;
        goto out;
    }
...
// pull stack item from task stack
void *task_get_stack_item(struct task *task, int index) {
    void *result = 0;

    uint32_t *sp_ptr = (uint32_t*)task->registers.esp; // making an array to stack ptr of cast
