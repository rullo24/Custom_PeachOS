...
const char *elf_signature[] = {0x7f, 'E', 'L', 'F'}; // 0x7f is a fixed value to ensure signature starts with a fixed, non-printable, unique-byte
static bool elf_valid_signature(void *buffer) {
    return memcmp(buffer, (void*)elf_signature, sizeof(elf_signature)) == 0;
}

// only support 32-bit binaries --> ensure .elf files are of this type
...
int elf_process_phdr_pt_load(struct elf_file *elf_file, struct elf32_phdr *phdr) {
    if (elf_file->virtual_base_address >= (void*)phdr->p_vaddr || elf_file->virtual_base_address == 0x0) {
    ...
    if (elf_file->virtual_end_address <= (void*)end_virtual_address || elf_file->virtual_end_address == 0x0) {
...
int elf_process_pheader(struct elf_file *elf_file) {
    ...
    for (int i=0; i < header->e_phnum; i++) {
        struct elf32_phdr *phdr = elf_program_header(header, i); // get program header from index
...
int elf_load(const char *filename, struct elf_file **file_out) {
    ...
    res = fread(elf_file->elf_memory, stat.filesize, 1, fd); // loading entire elf file into memory
    if (res < 0) {
        goto out;
    }
