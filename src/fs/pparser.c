...
static const char *pathparser_get_path_part(const char **path) {
    ...
    while (**path != '/' && **path != '\0') {
        ...
        *path += 1; // increment ptr by 1 byte
        i++;
    }
...
struct path_root *pathparser_parse(const char *path, const char *current_dir_path) {
    ...
    res = pathparser_get_drive_by_path(&tmp_path); // returns drive num if successful (else -ve)
    if (res < 0) {
        goto out;
    }
    ...
    struct path_part *first_part = pathparser_parse_path_part(NULL, &tmp_path); // NULL parsed as this is the first bit of path
    if (!first_part) {
        goto out;
    }
